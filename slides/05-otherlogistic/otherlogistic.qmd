...
- To quantify the uncertainty in estimating ED50 or LD50, we can use the [delta method](https://en.wikipedia.org/wiki/Delta_method#Multivariate_delta_method)
$$
...
- Standard logistic regression uses **maximum likelihood (ML)**. When the two outcome groups can be separated exactly by a linear combination of predictors, ML breaks down.

- **Complete separation** means that there exists a vector $\boldsymbol{\gamma}$ such that
$$
\mathbf{x}_i^T \boldsymbol{\gamma} > 0 \text{ for all } Y_i = 1, \qquad
\mathbf{x}_i^T \boldsymbol{\gamma} < 0 \text{ for all } Y_i = 0.
$$
That is, a hyperplane classifies all observations correctly.  
**Quasi-complete separation** is similar, except that some observations may lie exactly on the boundary.

- In either case, the fitted probabilities can be driven arbitrarily close to 1 for one class and 0 for the other. As a result, the log-likelihood keeps increasing as one or more coefficients go to $\pm \infty$, so the ML estimator is **not finite**.

- This is why `glm()` may show warnings such as non-convergence or fitted probabilities numerically equal to 0 or 1. The key message is:

> Perfect classification does **not** imply stable estimation.  
> It often means the ML coefficients are diverging, so Wald standard errors, $z$-tests, and confidence intervals become unreliable.

- Let's start with the famous Fisher iris data. We restrict to species `setosa` and `versicolor` and classify them using predictors `Sepal.Width` and `Sepal.Length`.
```{r}
(irisr <- iris %>%
  as_tibble() %>%
...
summary(lmod)
```
The residual deviance is essentially 0, indicating near-perfect classification, but the inferential output is misleading because the ML fit is on the boundary of the parameter space.

- A scatter plot shows that `setosa` and `versicolor` are perfectly separable by a straight line
$$
\eta = \beta_0 + x_{\text{Sepal.Width}} \beta_{\text{Sepal.Width}} + x_{\text{Sepal.Length}} \beta_{\text{Sepal.Length}} = 0.
$$
```{r}
irisr %>%
  ggplot() + 
...
- A principled remedy is **Firth logistic regression**. It modifies the score equations, equivalently maximizing the penalized log-likelihood
$$
\ell^*(\boldsymbol{\beta}) = \ell(\boldsymbol{\beta}) + \frac{1}{2} \log |I(\boldsymbol{\beta})|,
$$
where $I(\boldsymbol{\beta})$ is the Fisher information matrix.

- For teaching purposes, the main ideas are:
    1. it reduces the small-sample bias of ML estimates,
    2. it yields **finite coefficient estimates** under complete or quasi-complete separation,
    3. it typically shrinks extreme estimates away from $\pm \infty$, and
    4. inference is usually based on **penalized likelihood** rather than ordinary Wald approximations.

- In logistic regression, the mean bias-reduction approach and Firth's penalized-likelihood approach coincide, so the bias-reduced fit below can be presented as a **Firth-type fit**.
```{r}
library(brglm)
...
- Interpretation: the coefficients are now finite and the fitted boundary is well defined. The goal is no longer to force fitted probabilities to exactly 0 and 1, but to obtain a stable separating rule and valid uncertainty quantification.

- If you want to explicitly mention software that implements **Firth logistic regression**, a commonly used package is `logistf`, which also provides penalized profile-likelihood confidence intervals:
```{r, eval=FALSE}
library(logistf)

fmod <- logistf(Species ~ Sepal.Width + Sepal.Length, data = irisr)
summary(fmod)
confint(fmod)
```
