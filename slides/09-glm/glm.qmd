...
### Canonical link: direct derivation

- First consider a GLM with the canonical link. Then
$$
  \theta_i = \eta_i = \mathbf{x}_i^T\boldsymbol{\beta}.
$$
Substituting this directly into the exponential-family log-likelihood gives
\begin{eqnarray*}
  \ell(\boldsymbol{\beta})
  &=& \sum_{i=1}^n \left[\frac{y_i\mathbf{x}_i^T\boldsymbol{\beta} - b(\mathbf{x}_i^T\boldsymbol{\beta})}{a(\phi)} + c(y_i, \phi)\right].
\end{eqnarray*}
Differentiating with respect to $\boldsymbol{\beta}$,
\begin{eqnarray*}
  \nabla \ell(\boldsymbol{\beta})
  &=& \frac{1}{a(\phi)}\sum_{i=1}^n \left[y_i - b'(\mathbf{x}_i^T\boldsymbol{\beta})\right]\mathbf{x}_i \\
  &=& \frac{1}{a(\phi)}\sum_{i=1}^n (y_i - \mu_i)\mathbf{x}_i \\
  &=& \frac{1}{a(\phi)}\mathbf{X}^T(\mathbf{y}-\boldsymbol{\mu}).
\end{eqnarray*}
Thus the MLE satisfies
$$
  \mathbf{X}^T(\mathbf{y}-\widehat{\boldsymbol{\mu}})=\mathbf{0}.
$$
This has the same form as the normal equations in least squares: the residual vector is orthogonal to each column of $\mathbf{X}$.

- The Hessian is also simple under the canonical link:
\begin{eqnarray*}
  -\nabla^2\ell(\boldsymbol{\beta})
  &=& \frac{1}{a(\phi)}\sum_{i=1}^n b''(\theta_i)\mathbf{x}_i\mathbf{x}_i^T \\
  &=& \sum_{i=1}^n \frac{\sigma_i^2}{[a(\phi)]^2}\mathbf{x}_i\mathbf{x}_i^T.
\end{eqnarray*}
Equivalently,
$$
  -\nabla^2\ell(\boldsymbol{\beta}) = \mathbf{X}^T\mathbf{W}\mathbf{X},
  \qquad
  \mathbf{W}=\operatorname{diag}\left(\frac{b''(\theta_i)}{a(\phi)}\right)
  =\operatorname{diag}\left(\frac{\sigma_i^2}{[a(\phi)]^2}\right).
$$
Since $b''(\theta_i)\geq 0$, the negative Hessian is positive semidefinite.

### General link: where the extra factors enter

- For a non-canonical link, $\eta_i=g(\mu_i)$ but usually $\eta_i\neq \theta_i$. Then we cannot substitute $\theta_i=\mathbf{x}_i^T\boldsymbol{\beta}$ directly. The score follows from the chain rule:
$$
  \frac{\partial \ell_i}{\partial \boldsymbol{\beta}}
  =
  \frac{\partial \ell_i}{\partial \theta_i}
  \frac{d\theta_i}{d\mu_i}
  \frac{d\mu_i}{d\eta_i}
...
$$
  ...
  \qquad
  \frac{d\theta_i}{d\mu_i}=\frac{a(\phi)}{\sigma_i^2},
  \qquad
...
$$
For the canonical link, $\mu_i'(\eta_i)=d\mu_i/d\theta_i=\sigma_i^2/a(\phi)$, so this reduces to the canonical-link score derived above.

- Differentiating the general-link score gives
\begin{eqnarray*}
...
\end{eqnarray*}
For canonical links, the second and third terms cancel. To see this, note that under the canonical link $\mu_i'(\eta_i) = b''(\theta_i) = \sigma_i^2/a(\phi)$, and by the chain rule
$$
  \frac{d \sigma_i^2}{d \mu_i} = \frac{d \sigma_i^2}{d \theta_i} \cdot \frac{d \theta_i}{d \mu_i} = b'''(\theta_i) a(\phi) \cdot \frac{1}{b''(\theta_i)} = \frac{\mu_i''(\eta_i) \, a(\phi)}{\mu_i'(\eta_i)}.
$$
Substituting into the third term and using $\sigma_i^4 = [\mu_i'(\eta_i)]^2 [a(\phi)]^2$ gives $\frac{(y_i - \mu_i)\mu_i''(\eta_i)}{\sigma_i^2} \mathbf{x}_i \mathbf{x}_i^T$, exactly cancelling the second term.
The remaining first term is the $\mathbf{X}^T\mathbf{W}\mathbf{X}$ negative Hessian shown above.

- Fisher scoring replaces the observed information by the expected information matrix. Since $\mathbb{E}(y_i-\mu_i)=0$, the two residual-dependent terms vanish in expectation, giving
$$
...
$$
where $\mathbf{W} = \operatorname{diag}([\mu_i'(\eta_i)]^2/\sigma_i^2)$. The resulting modified Newton-Raphson algorithm is called the **Fisher scoring algorithm**. For canonical links, the expected information equals the observed information.
...
$$
  \Phi^{-1} \left( \frac{n+i}{2n + 1} \right), \quad i=1,\ldots,n.
$$
